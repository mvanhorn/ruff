...
When a typevar appears nested inside a covariant generic type in another constraint's bound,
transitivity should propagate through the nesting via the definition of covariance:
`A ≤ B → G[A] ≤ G[B]`.
...
When a typevar appears nested inside a contravariant generic type, the direction of bounds is
flipped: `A ≤ B → G[B] ≤ G[A]`.
